...
function doSearch() {
	try {
		...
		var spaceName = paramterMap　[SPACE_NAME_PARAMETER_NAME];
		...
		// 検索結果を取得
		searchresult = [];
		searchresult = search(regSearchWords, spaceName);
...
function traverseDom(root, a) {
	...
	do {
		...
		if (n == null || (c.nodeType == Node.ELEMENT_NODE && c.className == "relation") || checkSkipHighLight(n) || checkSkipHighLight(c)) {
			// visit c
            if (c.nodeType == Node.TEXT_NODE && chechTextIsBreakLine(c.nodeValue) == false) {
				a.push(c);
			}
			// done visit c
			n = c.nextSibling;
		...
		if (n == null) {
			...
			do {
				...
				try {
					// visit n
                    if (n.nodeType == Node.TEXT_NODE && chechTextIsBreakLine(c.nodeValue) == false) {
						a.push(n);
					}
				} catch (e) {
					n = root;
...
function chechTextIsBreakLine(text) {
//'22.03.16 KOD
//改行コード（CRLFとCR）の場合は処理をスキップする
    var pattern = /\r?\n|\r/g;
...
function search(searchWords, searchSpaceName)
{
	var spaceIndex = -1;
	for (i = 0; i < spaces.length; i++) {
		if(spaces[i][1] == searchSpaceName){
			spaceIndex = i;
		}
	}
	...
	for (i = 0; i < pageDataArray.length; i++)
	{
		...
		if (spaceIndex != -1 && spaceIndex != pageData[0])
		{
			continue;
		}
		var times = 0;
...
function renderResultView(resultArray, pageNumber) {
	...
	for (i = startIndex; i < endIndex; i++) {
		...
		resultHtml += '<h2>';
		if(spaces.length >  1 &&  isNullOrEmpty($("#" + SPACE_FIELD_ID).val())){
			resultHtml += '【<a class="skipHighlight" href = "';
			resultHtml += getLinkFollowSpace(spaces[searchResultData.spaceIndex][1]);
			resultHtml += '">' + spaces[searchResultData.spaceIndex][2] + '</a>】';
		}
		resultHtml += '<a href = "';
		resultHtml += src;
...
function doRenderResult(resultArray) {
	document.getElementById("resultSearchContainner").innerHTML = renderResultView(resultArray, eval($("#" + DISPLAYE_PAGE_INDEX_PARAMETER_NAME).val()));
